...
const Sidebar = () => {
  ...
  return (
   ...
   <div className="space-y-2.5 text-sm font-bold">
        <button className="flex items-center gap-3 text-white bg-[#2A40F4] px-3 py-2 rounded-md w-full">
        ...
        <SidebarItem icon={<FaLink />} label="Payment Links" />
...
const SidebarItem = ({ icon, label }) => {
  return (
    <div className="flex items-center gap-2 text-gray-500 hover:bg-gray-100 px-3 py-2 rounded-md cursor-pointer">
      {icon}
      <span>{label}</span>
    </div>
  );
